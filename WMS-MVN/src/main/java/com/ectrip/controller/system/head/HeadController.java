package com.ectrip.controller.system.head;
...
import com.ectrip.util.AppUtil;
import com.ectrip.util.Const;
import com.ectrip.util.Jurisdiction;
import com.ectrip.util.PageData;
import com.ectrip.util.SmsUtil;
import com.ectrip.util.Tools;
import com.ectrip.util.Watermark;
import com.ectrip.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",ectrip,"+pd.getString("PORT")+",ectrip,"+pd.getString("EMAIL")+",ectrip,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",ectrip,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",ectrip,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",ectrip,"+pd.getString("fcontent")+",ectrip,"+pd.getString("fontSize")+",ectrip,"+pd.getString("fontX")+",ectrip,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",ectrip,"+pd.getString("imgUrl")+",ectrip,"+pd.getString("imgX")+",ectrip,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",ectrip,"+pd.getString("WIMPORT")+",ectrip,"+pd.getString("OLIP")+",ectrip,"+pd.getString("OLPORT")+",ectrip,"+pd.getString("FHsmsSound"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
