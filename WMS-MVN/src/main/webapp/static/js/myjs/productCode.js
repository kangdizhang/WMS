 ...
 */	
	...
	function saveD(){
		...
		var fields = dname + ',ectrip,' + dtype + ',ectrip,' + dbz + ',ectrip,' + isQian + ',ectrip,' + ddefault + ',ectrip,' + flength;;
	...
	function editField(value,msgIndex){
		$("#dialog-add").css("display","block");
		var efieldarray = value.split(',ectrip,');
	...
	function appendC(value){
		var fieldarray = value.split(',ectrip,');
		$("#fields").append(
			'<tr>'+
	...
	function isSame(value){
		for(var i=0;i<arField.length;i++){
			var array0 = arField[i].split(',ectrip,')[0];
			if(array0 == value){
				return false;
			}
		}
